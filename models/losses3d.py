...
def calc_supervised_loss_3d(flows, target, cfgs, indices):
    if hasattr(cfgs, 'level_weights') and cfgs.level_weights != 'None':
        assert len(flows) <= len(cfgs.level_weights)
        level_weights = cfgs.level_weights
    else:
        decay = cfgs.iters_weight_decay
        level_weights = [decay**i for i in range(len(flows))]

    total_loss = 0
    for idx, (flow, level_weight) in enumerate(zip(flows, level_weights)):
        if target.shape[-1] == flow.shape[-1]:
            level_target = target
        else:
