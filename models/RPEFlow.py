...
class RPEFlow(nn.Module):
    ...
    def forward(self, inputs, is_Train=True):
        ...
        event_voxel = inputs['event_voxel']

        origin_h, origin_w = images.shape[2:]
        images = resize_to_64x(images, None)[0]
        event_voxel = resize_to_64x(event_voxel, None)[0]
        ...
        efeats_2d = self.pwc_fusion_core.encode_event(event_voxel)

        # predict flows (1->2)
        flows_2d, flows_3d, mi_loss = self.pwc_fusion_core.decode(
            xyzs1, xyzs2, feats1_2d, feats2_2d, feats1_3d, feats2_3d, efeats_2d,
        ...
        if not is_Train or 'flow_2d' not in inputs or 'flow_3d' not in inputs:
            return {
        ...
        final_loss_3d = calc_supervised_loss_3d(flows_3d, target_3d, self.cfgs.loss3d, sample_indices1) * 10
        final_mi_loss = mi_loss * 0.01
        self.loss = final_loss_2d + final_loss_3d + final_mi_loss

        # prepare scalar summary
        self.scalar_summary = {'loss': self.loss, 'loss_2d': final_loss_2d.item(), \
            'loss_3d': final_loss_3d.item()}
        if self.is_mi:
            self.scalar_summary['mi_loss'] = final_mi_loss.item()
    ...
    def get_log_string(self, scalar_summary=None):
        ...
        log_strings = [
            ...
            'loss_2d: %.3f' % scalar_summary['loss_2d'],
            'loss_3d: %.3f' % scalar_summary['loss_3d'],
        ]
        if self.is_mi:
            log_strings.append('mi: %.3f' % scalar_summary['mi_loss'])
