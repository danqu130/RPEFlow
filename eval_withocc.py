...
class Evaluator:
    ...
    def run(self):
        ...
        for inputs in self.test_loader:
            ...
            qbar.set_description('time: {:8.6f}, EPE2D: {:8.6f}, EPE3D: {:8.6f}, EPE3DNoOCC: {:8.6f}'.format(
                total_time,
                metrics_2d['EPE2d'] / metrics_2d['counts'], \
                metrics_3d['EPE3d'] / metrics_3d['counts'], \
                metrics_3d_noc['EPE3d'] / metrics_3d_noc['counts']))
            qbar.update()

        qbar.close()

        logging.info('#### Time ####')
        logging.info('Time: %.4f' % (np.mean(times)))
