...
def random_flip(image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, flip_mode, event=None):
    assert flow_3d.shape[1] <= 4
    assert flip_mode in ['lr', 'ud']
    image_h, image_w = image1.shape[:2]

    if np.random.rand() < 0.5:  # do nothing
        return image1, image2, pc1, pc2, flow_2d, flow_3d, event
    ...
    if event is not None:
        new_event = flip_image(event, flip_mode)

    return new_image1, new_image2, new_pc1, new_pc2, new_flow_2d, new_flow_3d, new_event


def crop_image_with_pc(image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, crop_window, event=None):
    ...
    return image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, event


def random_crop(image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, crop_size, event=None):
    assert flow_3d.shape[1] <= 4
    ...
    return crop_image_with_pc(image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, crop_window, event=event)
...
def random_scale(image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, scale_range, event=None):
    ...
    image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, event = crop_image_with_pc(
        image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, crop_window, event=event
    )
    ...
    if not event is None:
        event = cv2.resize(event, (image_w, image_h), interpolation=cv2.INTER_LINEAR)
        # assert NotImplementedError("Currently not support random resize for event data, please disable random_crop in conf.")

    return image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, event


def joint_augmentation(image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, cfgs, event=None):
    if not cfgs.enabled:
        if event is None:
    ...
    if cfgs.random_horizontal_flip.enabled:
        image1, image2, pc1, pc2, flow_2d, flow_3d, event = random_flip(
            image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, flip_mode='lr', event=event
        )

    if cfgs.random_vertical_flip.enabled:
        image1, image2, pc1, pc2, flow_2d, flow_3d, event = random_flip(
            image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, flip_mode='ud', event=event
        )

    if cfgs.random_crop.enabled:
        image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, event = random_crop(
            image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy,
            crop_size=cfgs.random_crop.crop_size, event=event
        )

    if cfgs.random_scale.enabled:
        image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, event = random_scale(
            image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy,
            scale_range=cfgs.random_scale.scale_range, event=event
        )
