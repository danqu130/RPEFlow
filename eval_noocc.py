...
class Evaluator:
    ...
    def run(self):
        ...
        qbar = tqdm(range(len(self.test_loader)))
        for inputs in self.test_loader:
            # image_names = inputs['image_names']
            inputs = copy_to_device(inputs, self.device)
            # outputs = self.model.forward(inputs)
            outputs = self.model.forward(inputs, is_Train=False)
            ...
            for batch_id in range(size_of_batch(inputs)):
                ...
                if epe3d_map[flow_3d_mask].mean().item() > 10:
                    # print(image_names[0][batch_id], image_names[1][batch_id])
                    print(epe3d_map[flow_3d_mask].mean().item(), flow_3d_pred.mean().item())
                    depth = inputs['depths'][batch_id]
                    pc = inputs['pcs'][batch_id]
                    print(torch.isnan(depth).float().mean(), torch.isinf(depth).float().mean(), \
                        torch.isnan(pc).float().mean(), torch.isinf(pc).float().mean())
            ...
            qbar.set_description('EPE2D: {:8.6f}, EPE3D: {:8.6f}'.format(
                metrics_2d['EPE2d'] / metrics_2d['counts'], \
                metrics_3d['EPE3d'] / metrics_3d['counts']))
            qbar.update()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='conf/test/things.yaml',
