...
def clip_grad_value_(parameters, clip_value) -> None:
    ...
    for p in filter(lambda p: p.grad is not None, parameters):
        if p.grad.data.min() < min:
            min = p.grad.data.min()
        if p.grad.data.max() > max:
            max = p.grad.data.max()
...
class Trainer:
    def __init__(self, device, cfgs):
        ...
        if device.index is None:
            ...
            if self.n_gpus > 1:
                init_process_group('nccl', 'tcp://localhost:6789' if cfgs.port == 'random' else 'tcp://localhost:{}'.format(cfgs.port), world_size=self.n_gpus, rank=self.device.index)
        ...
        if hasattr(self.cfgs.model, 'fusion'):
            logging.info('With fusion model: %s' % self.cfgs.model.fusion)
        ...
        self.optimizer, self.lr_scheduler, self.lrstep = optimizer_factory(\
            self.cfgs.training, self.model.named_parameters(), \
            last_epoch=self.curr_epoch - 1, train_loader_length=len(self.train_loader))
    ...
    def train_one_epoch(self):
        ...
        for i, inputs in enumerate(self.train_loader):
            ...
            # for name, param in self.ddp.named_parameters():
            #     if param.grad is None:
            #         print(name)

            self.optimizer.zero_grad()
            self.amp_scaler.scale(loss).backward()
            # torch.nn.utils.clip_grad_norm_(self.ddp.parameters(), max_norm=1.0)
            # torch.nn.utils.clip_grad_value_(self.ddp.parameters(), 1e10)
            # clip_grad_value_(self.ddp.parameters(), 1e8)
